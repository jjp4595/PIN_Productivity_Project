...
def attractivity(shape, population, income, education, idx):
    ...
    for i in range(len(idx)):
        ...
        #Income
        count = population['KS101EW0001'].values[idx[i]]
        count = count * income.iloc[idx[i]]['rank_1':'rank_9'].values
        ...
        #Education ---------------------------------------------------------------
        edu_counts.append(population['KS101EW0001'].values[idx[i]].astype(int))
        
        edu = education.iloc[idx[i]][[2,3,4,5,6,7]].values.astype(int)
        levels = [edu[0], edu[1]+edu[2], edu[3] + edu[4], edu[5]]
        edu = sum(education.iloc[idx[i]][[2,3,4,5,6,7]].values.astype(int))
